...
filename = 'Scotlandpoints.json'
Scotlandpoints <- paste(readLines(filename), collapse="")

films <- Scotlandpoints %>% as.tbl_json %>% gather_array %>%
  spread_values(
    title = jstring("title"),
...
cast <- Scotlandpoints  %>% as.tbl_json  %>% gather_array %>%
  spread_values(
    title = jstring("title"),
    director = jstring('director'),
    year = jstring('year'),
...
by_director %>% filter(n > 0) %>%
  ...
  coord_flip() +
  labs(title = 'Points scorers (number of times scored not points scored)') +
  theme_fivethirtyeight()
...
films_top_director_all %>%
  ...
  labs(title = 'Count by Minute', fill = '') + 
  theme_fivethirtyeight()
...
films_top_director_all %>%
  ...
  labs(title = 'Director Count by Year', fill = '') + 
  theme_fivethirtyeight() +
...
by_actor %>% filter(n > 1) %>%
  ...
  geom_bar(stat = "identity") +
  coord_flip() +
  labs(title = 'People on the field at more than 1 point scoring moment') +
  theme_fivethirtyeight()
...
actor_active_years %>% gather(caps, cap_year, start, end) %>%
  ...
  labs(title = "Times When Players Were On The Pitch For Points") +
  theme_fivethirtyeight()
