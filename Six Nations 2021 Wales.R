...
filename = 'Walespoints.json'
Walespoints <- paste(readLines(filename), collapse="")

films <- Walespoints %>% as.tbl_json %>% gather_array %>%
  spread_values(
    title = jstring("title"),
...
cast <- Walespoints  %>% as.tbl_json  %>% gather_array %>%
  spread_values(
    title = jstring("title"),
    director = jstring('director'),
    year = jstring('year'),
