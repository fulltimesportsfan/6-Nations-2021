...
filename = 'Englandpoints.json'
Englandpoints <- paste(readLines(filename), collapse="")

films <- Englandpoints %>% as.tbl_json %>% gather_array %>%
  spread_values(
    title = jstring("title"),
...
cast <- Englandpoints  %>% as.tbl_json  %>% gather_array %>%
  spread_values(
    title = jstring("title"),
    director = jstring('director'),
    year = jstring('year'),
