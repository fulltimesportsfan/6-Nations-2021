...
filename = 'Francepoints.json'
Francepoints <- paste(readLines(filename), collapse="")

films <- Francepoints %>% as.tbl_json %>% gather_array %>%
  spread_values(
    title = jstring("title"),
...
cast <- Francepoints  %>% as.tbl_json  %>% gather_array %>%
  spread_values(
    title = jstring("title"),
    director = jstring('director'),
    year = jstring('year'),
...
films %>% ggplot(aes(x = year)) +
  geom_bar() +
  labs(title = 'France points by time') + 
...
by_director %>% filter(n > 0) %>%
  ...
  coord_flip() +
  labs(title = 'France points scorers (times scored not points scored)') +
  theme_fivethirtyeight()
...
films_top_director_all %>%
  ...
  labs(title = 'France Scorers by Minute', fill = '') + 
  theme_fivethirtyeight()
...
films_top_director_all %>%
  ...
  labs(title = 'France Scores by Minute %', fill = '') + 
  theme_fivethirtyeight() +
...
by_actor %>% filter(n > 1) %>%
  ...
  geom_bar(stat = "identity") +
  coord_flip() +
  labs(title = 'France players on the field at more than 1 scoring moment') +
  theme_fivethirtyeight()
...
actor_active_years %>% gather(caps, cap_year, start, end) %>%
  ...
  labs(title = "France - When Players Were On The Pitch For Points") +
  theme_fivethirtyeight()
